export default {
  ...
  skill: {
    ...
    p4: '熟悉前端<b>单元测试</b>，为团队制定 Vue 组件单元测试规范及流程；',
    p5: '了解 <b>Gitlab CI</b> 流程配置，并配合 Jenkins 完成前端项目的<b>部署和上线</b>工作；',
    p6: '做政府项目，对于 <b>Web 安全</b>有比较严格和全面的实践；',
  ...
  proEdr: {
    name: 'EDR - （终端侦测与响应）平台',
    description:
      '该项目阐述EDR概念本身，通过探针实时监控用户终端的进程和文件，并实时上报汇总到服务端，最后统一在 Web 端展示给安全专员查看。实现了恶意代码预警和预测，极大地提高了客户的安全级别。我作为前端负责人，主要负责项目 Web 前端部分的开发和维护工作。',
    ...
    h2: '使用第三方 <b>vxe-table</b> 实现复杂表格（接口信息量大、展示内容较多、列数多，用户操作密集）并考虑定制化',
  ...
  proLarge: {
    ...
    d1: '性能相关优化：为减少加载时间，减少图片使用，采用CSS绘制和SVG来呈现',
    d2: 'ATT&CK 热力图实现：同时考虑到小屏提供了simple模式展示',
    d3: '响应式设计改进：针对lg尺寸屏幕，对非核心展示元素进行缩略处理，既让页面简洁，又保留信息，得到甲方好评',
    h1: '首屏加载，白屏优化，相关性能优化',
  ...
  thanks: {
    title: '致谢',
    p1: '感谢您花时间阅读我的简历，我是刘放 ^_^ 一个喜欢篮球、动漫和音乐的前端工程师。',
